...
function AssociateLogin({ onLogin, onBack, onSignUp }) {
  ...
  // Set the page title
  useEffect(() => {
    document.title = "Associate Login | CV Unlocked";
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/login', {
        ...
        body: JSON.stringify({ email, password, role: 'associate' })
      });
      ...
      if (data.status === 'success') {
        ...
        setError(data.message || 'Invalid associate credentials.');
        setShowToast(true);
      }
    } catch (err) {
    ...
    <div className="sa-login-background">
      ...
      <main className="sa-login-main-container">
        ...
        <div className="sa-form-card">
          ...
          {/* Headings */}
          <h1 className="sa-login-title-alt">Associate Login</h1>
          <p className="sa-login-subtitle-alt">Secure access to daily operations</p>
          ...
          <div className="sa-form-container">
            <form onSubmit={handleSubmit}>
              ...
              <button type="submit" className="sa-btn-alt" disabled={loading}>
                {loading ? 'Verifying...' : 'Access Associate Dashboard'}
              </button>
            </form>
            <p style={{ textAlign: 'center', marginTop: '1.5rem', color: '#64748b', fontSize: '0.9rem' }}>
              Don't have an associate account? <a href="#" onClick={onSignUp} style={{ color: 'var(--primary-500)', fontWeight: '600', textDecoration: 'none' }}>Sign up</a>
            </p>
          </div>
        </div>
